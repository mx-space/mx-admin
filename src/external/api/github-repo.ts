...
export const getRepoDetail = (owner: string, repo: string) => {
  return fetch(`${endpoint}repos/${owner}/${repo}`).then(
    (res) => res.json() as Promise<IGithubRepo>,
  )
}
    ...
    .then((res) => {
      if (res) {
        ...
        const url = (() => {
          ...
          return `https://fastly.jsdelivr.net/gh/${owner}/${repo}@${branch}/${filename}`
        })()
